...
function updateFile(filePath) {
    try {
        ...
        fetchPatterns.forEach(({ pattern }) => {
            ...
            const hasPattern = patternClone.test(content);
            if (hasPattern) {
                needsImport = true;
        ...
        fetchPatterns.forEach(({ pattern, replacement }) => {
            // Create a new RegExp instance to avoid issues with lastIndex
            const patternClone = new RegExp(pattern.source, pattern.flags);

            // Use a callback to track replacements since we need a global regex
            const matches = [];
            content.replace(patternClone, (match) => {
                matches.push(match);
                return match;
            });
            ...
            if (matches.length > 0) {
                ...
                const newContent = content.replace(patternClone, replacement);
        ...
        if (modified) {
            ...
            console.log(`No changes needed for: ${filePath} (already has correct imports/patterns)`);
        }
    } catch (error) {
